...
func ApplyTags(filePath string, release Release, track Track) error {
	// Check if FLAC tools are installed
	if _, err := exec.LookPath("metaflac"); err != nil {
		return fmt.Errorf("metaflac.exe not found. Please install FLAC from https://xiph.org/flac/download.html or run: winget install FLAC")
	}
